...
| 변수 | 기본값 | 설명 |
|---|---:|---|
| `DEMO_IDLE_MINUTES` | `5` | 요청이 없는 데모 세션 만료 기준 |
| `DEMO_MAX_MINUTES` | `40` | 데모 계정 최대 수명(시작 시점 기준) |
| `DEMO_CLEANUP_INTERVAL_SECONDS` | `60` | 만료 계정 정리 주기 |
| `DEMO_CLOSE_GRACE_SECONDS` | `60` | 창 종료 후 재접속을 기다리는 삭제 유예 시간 |
| `DEMO_MAX_ACTIVE_SESSIONS` | `50` | 동시 활성 데모 사용자 수 |
...
브라우저 창을 닫으면 삭제가 예약되며, 유예 시간 안에 새로고침하거나 다시 접속하면 예약이 취소됩니다.
