      ...
      <div className="mx-auto max-w-[1600px] px-3 py-4 sm:px-6 sm:py-5 lg:px-8">
        <header className="mb-5 border-b border-slate-800 pb-5">
          <div className="flex flex-wrap items-end justify-between gap-4">
            ...
            <div className="ml-auto grid w-full grid-cols-[minmax(0,1fr)_auto_auto_auto] items-center gap-1 sm:w-auto sm:flex sm:flex-wrap sm:gap-2">
              <label className="sr-only" htmlFor="site-select">관리 현장</label>
              <select
                id="site-select"
                value={currentSite?.id ?? ''}
                onChange={(event) => selectSite(Number(event.target.value))}
                className="min-h-9 min-w-0 w-full rounded-lg border border-slate-700 bg-slate-900 px-2 py-1.5 text-xs text-white outline-none focus:border-cyan-500 sm:min-h-11 sm:w-auto sm:px-3 sm:py-2 sm:text-sm"
              >
                {session.sites.map((site) => <option key={site.id} value={site.id}>{site.name}</option>)}
              </select>
              <button onClick={() => setAddingSite((value) => !value)} className="min-h-9 whitespace-nowrap rounded-lg border border-cyan-500/40 px-2 py-1.5 text-xs text-cyan-300 hover:bg-cyan-500/10 sm:min-h-11 sm:px-3 sm:py-2 sm:text-sm">현장 추가</button>
              <button onClick={onShowRanking} className="min-h-9 whitespace-nowrap rounded-lg border border-emerald-500/40 bg-emerald-500/10 px-2 py-1.5 text-xs font-semibold text-emerald-300 hover:bg-emerald-500/20 sm:min-h-11 sm:px-3 sm:py-2 sm:text-sm">오늘 안전 순위</button>
              <button onClick={logout} className="min-h-9 whitespace-nowrap rounded-lg border border-slate-700 px-2 py-1.5 text-xs text-slate-400 hover:border-red-500/50 hover:text-red-300 sm:min-h-11 sm:px-3 sm:py-2 sm:text-sm">로그아웃</button>
            </div>
          </div>
