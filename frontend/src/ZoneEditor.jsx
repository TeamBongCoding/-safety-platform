...
export default function ZoneEditor({
  ...
  return (
    <>
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 bg-slate-950/40 px-3 py-2.5 sm:px-4">
        <div className="flex flex-wrap items-center gap-2 text-xs">
          <button type="button" onClick={setGlobalVisibility} className={`min-h-10 rounded-md border px-2.5 py-1.5 ${overlayVisible ? 'border-cyan-500/40 bg-cyan-500/10 text-cyan-300' : 'border-slate-700 text-slate-400'}`}>
        ...
        <button type="button" onClick={beginCreate} disabled={!streamReady || editorOpen} className="min-h-10 rounded-md bg-cyan-500 px-3 py-1.5 text-xs font-semibold text-slate-950 hover:bg-cyan-400 disabled:cursor-not-allowed disabled:opacity-40">
          새 위험구역 설정
        </button>
      </div>
