...
from app.time_utils import utc_naive_to_kst_isoformat
...
class TestRiskAPIAuth(unittest.TestCase):
    def setUp(self):
        ...
        db = Session()
        self.user, self.site = _setup_user(
            db,
            "test@example.com",
            "password123",
        )
    ...
    def test_risk_config_matches_environment(self):
        ...
        self.assertEqual(
            [item["value"] for item in data["options"]],
            ["24h", "7d"],
        )

        expected_labels = (
            [
                f"{RISK_SHORT_WINDOW_MINUTES}분",
                f"{RISK_LONG_WINDOW_MINUTES}분",
            ]
            if RISK_WINDOW_MODE == "demo"
            else ["24시간", "7일"]
        )
        self.assertEqual(
            [item["label"] for item in data["options"]],
            expected_labels,
        )
    ...
    def test_resolve_episode_of_other_site_rejected(self):
        ...
        resp = self.client.patch(
            f"/api/events/episodes/{episode_id}/resolve",
            json={"note": "침해 시도"},
            cookies={"safety_session": self.token},
        )
    ...
    def test_knowledge_chunk_requires_auth_and_site_access(self):
        ...
        unauth = self.client.get(
            f"/api/knowledge/chunks/{own_chunk_id}"
        )
        ...
        forbidden = self.client.get(
            f"/api/knowledge/chunks/{other_chunk_id}"
        )
...
class TestRiskTimestampSerialization(unittest.TestCase):
    def test_utc_stored_timestamp_is_converted_to_kst(self):
        value = utc_naive_to_kst_isoformat(
            datetime(2026, 8, 14, 1, 7, 50)
        )
        self.assertEqual(
            value,
            "2026-08-14T10:07:50+09:00",
        )
