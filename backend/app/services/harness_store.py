...
_devices: dict[tuple[int, str], dict] = {}
STALE_SEC = 5   # 이 시간 이상 수신 없으면 상태 불명 처리


def update(site_id: int, device_id: str, hook_closed: bool, rfid_tag: str | None):
    _devices[(site_id, device_id)] = {
...
def get(site_id: int, device_id: str = "worker-1") -> dict:
    d = _devices.get((site_id, device_id))
