...
from ..config import (
    ...
    OVERLAP_SCORE_MARGIN,
    POSE_TRACK_HOLD_FRAMES,
    POSE_TRACK_HOLD_SECONDS,
    POSE_LOW_PROFILE_RATIO,
    POSE_LOW_PROFILE_HOLD_FRAMES,
    POSE_LOW_PROFILE_HOLD_SECONDS,
    REID_DEEP_WEIGHT,
    ...
    TRACK_APPEARANCE_MATCH_THRESHOLD,
    TRACK_OCCLUSION_IOU,
    TRACK_STATIONARY_DISTANCE,
    TRACK_STATIONARY_MAX_MISSED_FRAMES,
)
...
def _maximum_weight_assignment(
    ...
    """Return a globally optimal one-to-one assignment using Hungarian matching."""
    if row_count == 0 or column_count == 0 or not weighted_pairs:
        return []
    ...
    for score, row, column in weighted_pairs:
    ...
    # 1-indexed shortest augmenting-path implementation.
    u = [0.0] * (size + 1)
    v = [0.0] * (size + 1)
    p = [0] * (size + 1)
    way = [0] * (size + 1)
    ...
    for column in range(1, size + 1):
        row = p[column] - 1
        col = column - 1
        if row < row_count and col < column_count and (row, col) in valid:
...
class GlobalIdentityManager:
    ...
    def __init__(self):
        ...
        # Merge only after the same cross-camera pair is confirmed repeatedly.
        self._overlap_confirmations: dict[tuple[str, str], tuple[int, float]] = {}
    ...
    def _canonical_active_in_camera_nolock(
        self,
        global_person_id: str,
        camera_id: int | None,
        timestamp: float,
        ...
        return any(
            self._resolve_id_nolock(info.global_person_id) == canonical
            and timestamp - info.timestamp <= OVERLAP_TIME_TOLERANCE
            for info in self._active_tracks.get(cam_key, {}).values()
        )

    def _ids_coexist_same_camera_nolock(
        self,
        left_id: str,
        right_id: str,
        timestamp: float,
    ) -> bool:
        left = self._resolve_id_nolock(left_id)
        right = self._resolve_id_nolock(right_id)
        for tracks in self._active_tracks.values():
            active_ids = {
                self._resolve_id_nolock(info.global_person_id)
                for info in tracks.values()
                if timestamp - info.timestamp <= OVERLAP_TIME_TOLERANCE
            }
    ...
    def _match_pending(
        ...
        for index, candidate in enumerate(self._pending):
            if candidate.source_camera_id == camera_id:
                continue
            if self._canonical_active_in_camera_nolock(
                candidate.global_person_id, camera_id, timestamp
            ):
                continue
        ...
        with self._lock:
            ...
            for other_cam_key, other_tracks in self._active_tracks.items():
                ...
                for other_local_id, other_info in other_tracks.items():
                    ...
                    if score >= REID_OVERLAP_THRESHOLD:
                        candidates.append((score, other_canonical))

            if not candidates:
                return None

            candidates.sort(reverse=True)
            best_score, best_other_canonical = candidates[0]
            if (
                len(candidates) > 1
                and best_score - candidates[1][0] < OVERLAP_SCORE_MARGIN
            ):
                return None
            if self._ids_coexist_same_camera_nolock(
                canonical, best_other_canonical, timestamp
            ):
                return None

            pair_key = tuple(sorted((canonical, best_other_canonical)))
            confirmation_count, last_confirmation = self._overlap_confirmations.get(
                pair_key, (0, float("-inf"))
            )
            # Both sides may call this method for the same video frame.
            if timestamp - last_confirmation >= 0.02:
                confirmation_count += 1
                last_confirmation = timestamp
            self._overlap_confirmations[pair_key] = (
                confirmation_count, last_confirmation
            )
            stale_before = timestamp - max(OVERLAP_TIME_TOLERANCE * 2, 1.0)
            self._overlap_confirmations = {
                key: value
                for key, value in self._overlap_confirmations.items()
                if value[1] >= stale_before
            }
            if confirmation_count < OVERLAP_CONFIRM_FRAMES:
                return None
            ...
            self._overlap_confirmations.pop(pair_key, None)
            return keep
...
class LocalTrack:
    ...
    pose_supported: bool = False
    behavior_hold: bool = False
    low_profile_candidate: bool = False
...
class CameraPersonTracker:
    ...
    def update(
        ...
        # Backward compatibility for callers written before overlap_zones was
        # inserted ahead of timestamp in this method's arguments.
        if isinstance(overlap_zones, (int, float)) and timestamp is None:
            timestamp = float(overlap_zones)
            overlap_zones = None
        ...
        # Overlapping crops can contain another person. Mark them so their
        # contaminated appearance does not overwrite a stable identity.
        for left_index, left in enumerate(observations):
            for right in observations[left_index + 1:]:
                if _iou(left["detection"]["box"], right["detection"]["box"]) >= TRACK_OCCLUSION_IOU:
                    left["ambiguous"] = True
        ...
        for track_index, local_id in enumerate(track_ids):
            ...
            for observation_index, observation in enumerate(observations):
                ...
                anchor_distance = math.hypot(
                    observation["point"][0] - track.point[0],
                    observation["point"][1] - track.point[1],
                )
                overlap = _iou(track.box, box)
                appearance = _cosine_similarity(track.embedding, observation["embedding"])
                appearance_weight = 0.42 if observation["ambiguous"] else 0.62
                score = (
                    0.12 * overlap
                    + appearance_weight * appearance
                    + 0.10 * max(0.0, 1.0 - distance / 0.35)
                    + 0.10 * max(0.0, 1.0 - anchor_distance / 0.20)
                )
                if anchor_distance <= TRACK_STATIONARY_DISTANCE:
                    score += 0.06
                if (
                    appearance >= TRACK_APPEARANCE_MATCH_THRESHOLD
                    or overlap >= 0.05
                    or distance <= 0.14
                    or anchor_distance <= 0.10
                ):
        ...
        assignments = _maximum_weight_assignment(len(track_ids), len(observations), pairs)
        for track_index, observation_index in assignments:
            local_id = track_ids[track_index]
            track = self._tracks[local_id]
            ...
            track.pose_supported = (
                track.pose_supported or detection.get("source") == "pose_fallback"
            )
            detected_width = max(1.0, detection["box"][2] - detection["box"][0])
            detected_height = max(1.0, detection["box"][3] - detection["box"][1])
            track.low_profile_candidate = (
                detection.get("source") == "pose_fallback"
                and detected_width / detected_height >= POSE_LOW_PROFILE_RATIO
            )
        ...
        for local_id, track in list(self._tracks.items()):
            if local_id in matched_tracks:
                continue
            track.missed += 1
            recent_points = track.points[-6:]
            stationary = (
                len(recent_points) >= 3
                and math.hypot(
                    recent_points[-1][0] - recent_points[0][0],
                    recent_points[-1][1] - recent_points[0][1],
                ) <= TRACK_STATIONARY_DISTANCE
            )
            missed_limit = (
                TRACK_STATIONARY_MAX_MISSED_FRAMES
                if stationary or track.pose_supported
                else TRACK_MAX_MISSED_FRAMES
            )
        ...
        for index, observation in enumerate(observations):
            ...
            new_track = LocalTrack(
                ...
                pose_supported=detection.get("source") == "pose_fallback",
                low_profile_candidate=(
                    detection.get("source") == "pose_fallback"
                    and max(1.0, detection["box"][2] - detection["box"][0])
        ...
        visible_track_ids = set(matched_tracks)
        for local_id, track in self._tracks.items():
            enhanced_hold = track.behavior_hold or track.low_profile_candidate
            hold_frames = (
                POSE_LOW_PROFILE_HOLD_FRAMES
                if enhanced_hold
                else POSE_TRACK_HOLD_FRAMES
            )
            hold_seconds = (
                POSE_LOW_PROFILE_HOLD_SECONDS
                if enhanced_hold
                else POSE_TRACK_HOLD_SECONDS
            )
            if (
                enhanced_hold
                and track.missed <= hold_frames
                and timestamp - track.last_observed_at <= hold_seconds
            ):
                visible_track_ids.add(local_id)
        return [
            self._tracks[local_id]
            for local_id in sorted(visible_track_ids)
